...
def e001():
    ...
    sh = X.Sheet("E-001", "SINGLE LINE DIAGRAM - SOURCES, BOARDS AND "
                 ...
                 "ONE BATTERY",
                 flags=("EL-V1", "EL-V3", "EL-V4"), sheet_of="1 OF 1")
    ...
    sh.panel_column(540.0, TOP, 122.0, 290.0, [
        ...
        ("EL-V1  -  THE QUESTION THAT SIZES THE BATTERY", [
            "MAY THE GENERATOR RUN DURING MODE 3 CLOSED?",
            "The project states BOTH, in the SAME schedule  [C]:",
            ...
            "BV-4 and BV-5 are two of the five.  BOTH CANNOT HOLD",
            "DURING MODE 3, and nothing says which gives way.  [U]",
            "",
            f"   CASE A   {P.CASE_A_H:.0f} h   {ah_a:6.0f} Ah   {kwh_a*1000/P.WH_PER_KG:5.0f} kg   a CABINET",
            f"   CASE B  {P.CASE_B_H:.0f} h   {ah_b:6.0f} Ah   {kwh_b*1000/P.WH_PER_KG:5.0f} kg   a ROOM",
            "",
            f"{ah_b/ah_a:.0f}x APART.  Case B needs {kwh_b*1000/P.WH_PER_KG*9.81/1000/P.FLOOR_LL_KPA:.1f} m2 of floor just to stay",
            f"inside the {P.FLOOR_LL_KPA:.0f} kPa floor live load  [C] A.7.2 - AND BAY 5",
            "IS ALREADY 80 % OCCUPIED (CO-3).  NOWHERE TO PUT IT.",
            "",
            "CASE A ADOPTED  [A], because it is the reading the",
            "project's OWN document implies - mode 5 is called",
