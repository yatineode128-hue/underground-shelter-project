...
def main():
    ...
    # ------------------------------------------------------------- E.4
    h1("E.4", "THE QUESTION THAT DECIDED THE PACKAGE  -  NOW RULED")
    w("")
    w("  MAY THE GENERATOR RUN DURING MODE 3 CLOSED?")
    w("")
    w("  *** RULED YES - RC2, 11 September 2026, master H.21. ***")
    w("  The project owner has ruled: ALL FIVE VALVES SHUT AT THE SHOCK AND")
    w("  HOLD 1.3 s; BV-4 AND BV-5 ARE THEN REOPENED FOR GENERATOR OPERATION.")
    w("  Bay 8 is outside the gas-tight envelope, so the clean zone is")
    w("  unaffected - which is what the mode 5 note always said.  HV1's mode")
    w("  schedule is amended to match (RC2).  CASE A IS CONFIRMED.")
    w("")
    w("  The reasoning that produced the question is kept below, because it")
    w("  is what made the question answerable - and because the size of the")
    w("  consequence is the reason the ruling mattered.")
    w("")
    w("  AS ISSUED, the project stated BOTH of these in the SAME schedule:")
    w("")
    ...
    w("")
    w("  BV-4 and BV-5 are two of the five.  Mode 3 shut them; mode 5 needed")
    w("  them open.  The two statements could not both hold during mode 3, and")
    w("  THE PROJECT HELD NO POSITION.  That was EL-V1.  ** IT IS NOW RULED:")
    w("  CASE A.  HV1's mode 3 row is amended to match. **")
    w("")
    w("  Both readings were defensible, which is why it needed a ruling:")
    ...
    w("            everything.")
    w("")
    w("  E.5 sizes both.  THE ANSWER DIFFERED BY A FACTOR OF TWELVE - which is")
    w("  why this was worth putting to the owner rather than assuming.  [D]")
    ...
    w("")
    w(f"  CASE A CONFIRMED: {P.CASE_A_H:.0f} h, {ah_a:.0f} Ah at {P.V_DC:.0f} V.  [C] RULED - RC2,")
    w("  11 September 2026, master H.21.  EL1 had already adopted Case A as [A],")
    w("  reasoning that it is the reading the project's OWN document implies -")
    w("  the mode schedule calls mode 5 'power OR BATTERY CHARGING' and says it")
    w("  is 'independent of modes 1-4'.  THE RULING CONFIRMS THAT READING.")
    w("")
    w("  ** NO NUMBER IN THIS PACKAGE CHANGES.  The battery stands at the size")
    w("     EL1 designed; only its evidence class moves from [A] to [C].  That")
    w("     is the best outcome a ruling can have - it confirms the design")
    w("     rather than replacing it.  EL-V1 IS CLOSED. **")
    w("")
    w("  ONE CONSEQUENCE, RECORDED NOT RESOLVED:  reopening BV-4 and BV-5")
    w("  leaves TWO DN350 BORES OPEN in the post-attack period - the two that")
    w("  EM1 showed FAIL BOTH EMP CRITERIA (cutoff 502 MHz, 54.8 dB).  That")
    w("  SHARPENS EM-V3 - whether bay 8 is inside the EMP boundary - without")
    w("  deciding it.  [D]")
